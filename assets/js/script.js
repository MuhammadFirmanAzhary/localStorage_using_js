...
const ovf = document.getElementById("overlayForm");
...
document.addEventListener(RENDER_EVENT, () => {
  ...
  for (const book of storeData) {
    ...
    if (!book.isCompleted) unreadBook.append(bookElement);
...
function newBook(bookObject) {
  ...
  if (bookObject.isCompleted) {
    image.setAttribute("src", "assets/images/before.jpg");
  } else {
    image.setAttribute("src", "assets/images/after.jpg");
  }
...
function loadData() {
  ...
  document.dispatchEvent(new Event(RENDER_EVENT));
  unreadBookLength(); // Memanggil fungsi untuk menghitung buku yang belum dibaca
  readBookLength();
}
